...
void glLoadMatrixd(const GLdouble *m) {
    assert_msg(!gl_prim_type, "Not allowed within glBegin/glEnd pair.");
    {
	    register double *o=(double *)m;
	    register float *d=(float *)&ml;
	    register int i;
	    for(i=0;i<16;i++)
		    *d++=*o++;
    }
...
static __inline__ void _gluMultMatrixVecd(const GLfloat matrix[16], const GLfloat in[4],
                               GLfloat out[4])
{
    int i;
...
static __inline__ void _gluMultMatricesd(const GLfloat a[16], const GLfloat b[16], GLfloat r[16])
{
    int i, j;
...
static __inline__ int _gluInvertMatrixd(const GLfloat src[16], GLfloat inverse[16])
{
    int i, j, k, swap;
    float t;
    GLfloat temp[4][4];
