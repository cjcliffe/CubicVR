/* KGL-X 0.0
   ...
   based on code from TinyGL  © 1997-1998 Fabrice Bellard
   and TinyGL AmigaDE  © 2002 Ruben Monteiro
...
/*
static __inline__ void *gl_malloc(int size)
{
    return malloc(size);
}
*/

static __inline__ void *gl_zalloc(int size)
{
...
static void delete_list(int list)
{
    ...
    if(l!=NULL) {
    	GLParamBuffer *pb1, *pb=l->first_op_buffer;
    	while (pb!=NULL) {
        	pb1=pb->next;
        	gl_free(pb);
        	pb=pb1;
...
void glNewList( GLuint list, GLenum mode )
{
    ...
    assert(list && list<=MAX_DISPLAY_LISTS);
    list--;
